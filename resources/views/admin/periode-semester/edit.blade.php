...
<div class="min-h-screen p-6 bg-slate-50">
    <div class="max-w-2xl mx-auto">
        <div class="mb-6">
            ...
            <h1 class="text-2xl font-bold text-slate-800 m-0">Edit Periode Semester</h1>
        </div>

        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <form action="{{ route('admin.periode-semester.update', $periode->id) }}" method="POST" class="p-6 space-y-5">
                @csrf @method('PUT')
                <div class="bg-emerald-50 border border-emerald-200 rounded-lg p-3 text-sm text-emerald-700">
                    <strong>Nama Periode:</strong> {{ $periode->nama_periode }} (otomatis dihasilkan dari Semester + Tahun Ajaran)
                </div>
                <div class="grid grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input type="text" name="tahun_ajaran" value="{{ old('tahun_ajaran', $periode->tahun_ajaran) }}" required 
